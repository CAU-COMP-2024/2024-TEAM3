...
app.post('/find-password', (req, res) => {
    const { email, nickname } = req.body; // 요청에서 이메일과 닉네임 추출

    // 입력값 검증
    if (!email || !nickname) {
        return res.status(400).send('이메일과 닉네임을 입력해주세요.');
    }

    const sql = 'SELECT password FROM users WHERE email = ? AND nickname = ?'; // SQL 쿼리

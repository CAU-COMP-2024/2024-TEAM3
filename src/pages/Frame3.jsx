import { useCallback } from "react";
import { useNavigate } from "react-router-dom";
import styles from "./Frame3.module.css";

const Frame3 = () => {
  const navigate = useNavigate();

  const onRectangleClick = useCallback(() => {
    navigate("/");
  }, [navigate]);

  const onRectangleClick1 = useCallback(() => {
    navigate("/1");
  }, [navigate]);

  const onRectangleClick2 = useCallback(() => {
    navigate("/-things-to-do");
  }, [navigate]);

  const onRectangleClick3 = useCallback(() => {
    navigate("/-all-about-exam");
  }, [navigate]);

  return (
    <div className={styles.div}>
      <div className={styles.div1} />
      <div className={styles.div2}>
        <div className={styles.allHere}>ALL HERE</div>
        <div className={styles.div3}>오리어</div>
        <div className={styles.child} />
        <img className={styles.comp21} alt="" src="/comp-2--1@2x.png" />
      </div>
      <div className={styles.item} />
      <div className={styles.div4}>
        <div className={styles.div5}>계정</div>
        <div className={styles.div6}>설정</div>
        <div className={styles.inner} />
        <img className={styles.icon} alt="" src="/@2x.png" />
        <img className={styles.icon1} alt="" src="/1@2x.png" />
        <div className={styles.rectangleDiv} onClick={onRectangleClick} />
        <div className={styles.child1} />
      </div>
      <div className={styles.div7}>
        <div className={styles.child2} onClick={onRectangleClick1} />
        <div className={styles.div8}>개인용</div>
        <div className={styles.div9}>팀플용</div>
        <div className={styles.thingsToDo}>Things to do</div>
        <div className={styles.thingsToDo}>Things to do</div>
        <div className={styles.div10}>프로젝트 1</div>
        <div className={styles.div11}>프로젝트 2</div>
        <div className={styles.allAboutExam}>All about exam</div>
        <div className={styles.div12}>과목 1</div>
        <div className={styles.div13}>역할 분담</div>
        <div className={styles.div14}>{`일정 `}</div>
        <div className={styles.div15}>회의</div>
        <div className={styles.div16}>문서</div>
        <div className={styles.div17}>과목 2</div>
        <div className={styles.div18}>과목 3</div>
        <img className={styles.icon2} alt="" src="/-@2x.png" />
        <img className={styles.icon3} alt="" src="/2@2x.png" />
        <div className={styles.child3} onClick={onRectangleClick2} />
        <div className={styles.child4} onClick={onRectangleClick3} />
        <div className={styles.child5} onClick={onRectangleClick3} />
        <div className={styles.child6} onClick={onRectangleClick1} />
      </div>
      <div className={styles.child7} />
      <div className={styles.mainContainer}>
        <span className={styles.main}>MAIN</span>
        <span className={styles.span}> 개인용 팀플용</span>
      </div>
      <div className={styles.welcomeUserName}>Welcome USER NAME!</div>
      <div className={styles.div19}>오늘까지인 일정 1개</div>
      <div className={styles.child8} />
      <div className={styles.child9} />
      <div className={styles.child10} />
    </div>
  );
};

export default Frame3;
